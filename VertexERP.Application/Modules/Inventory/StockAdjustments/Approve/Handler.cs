...
public sealed class Handler(IAppDbContext dbContext , ICurrentUser currentUser) : IHandler
{
    public async Task<Result<Response>> HandleAsync(Guid id ,Request request, CancellationToken ct)
    {
        ...
        if (adjustment is null)
            return Result<Response>.NotFound( "Stock adjustment was not found.");

        var result = adjustment.Approve(currentUser.UserId);
        ...
        return Result<Response>.Success(response, "Stock adjustment approved successfully.");
    }
}